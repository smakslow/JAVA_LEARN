...
public class FileRename {
    ...
    static String oldString = ".avi";//要被替换的字符串
    static String dir = "I:\\亿级流量电商详情页系统实战（第二版）：缓存架构+高可用服务架构+微服务架构";//文件所在路径，所有文件的根目录，记得修改为你电脑上的文件所在路径
                ...
                for (File file : fileArr) {
                    if (file.isDirectory()) {//是文件夹，继续递归，如果需要重命名文件夹，这里可以做处理
                        ...
                        if (fileName.contains(oldString)) {//文件名包含需要被替换的字符串
                            ...
                            newDir = new File("I:\\亿级流量视频" + "/" + newName);//文件所在文件夹路径+新文件名
